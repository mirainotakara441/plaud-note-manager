    ...
    <main className="mx-auto flex min-h-[80vh] max-w-sm flex-col justify-center px-6 pt-[env(safe-area-inset-top)]">
      ...
      >
        <input
          ...
          placeholder="合言葉"
          disabled={loading}
          className="block w-full rounded-xl border border-gray-300 px-4 py-3 text-base text-gray-900 focus:border-teal-500 focus:outline-none focus:ring-2 focus:ring-teal-200 disabled:opacity-50"
        />
        <button
          type="submit"
          disabled={loading || !passphrase.trim()}
          className="w-full rounded-xl bg-teal-700 px-4 py-3 text-base font-semibold text-white transition active:bg-teal-800 disabled:opacity-40"
        >
          {loading ? "確認中..." : "入る"}
        </button>
        {error && (
