...
作成: 2026-07-30 / 状態: **全手順 完了（1-b の完了は 2026-09-12 に本番実測で確認）**
...
| 手順1-b 制約の締め | ✅ **完了** | 2026-09-12 の総点検フォローで本番の `pg_get_constraintdef` を実測したところ、**既に締まった形**（8値のみ・`委託企業` なし）だった。実行された記録がこの文書に反映されないまま「未実行」表記が1ヶ月半残っていた。データ側も `委託企業` 0行・`委託会社` 10行を確認。※「文書が未実行と言っていても本番を数えてから動く」——proposal_cache の誤記載と同じ教訓 |
