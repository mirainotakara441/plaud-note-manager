...
//
// 週報は organization 完全一致（タイムラインと同じ）で拾う。
// 以前は ILIKE 部分一致だったため「横浜市」で「尾崎横浜市議会議員」のような
// 近縁レコードまで混ざっていた（2026-08-19 総点検で修正）。
// exact / pickHeadline は当時の部分一致ヒットを区別する仕組みの名残りだが、
// 完全一致のみを返すようになった今も無害に動く（常に exact: true になる）ため残してある。
