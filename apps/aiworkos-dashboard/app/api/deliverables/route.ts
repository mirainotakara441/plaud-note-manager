...
// 種別。以前は相手先（社内/社外）で選択肢を分けていたが、5種類に統一した
// （2026-08-23）。過去に登録した成果物（実習書・報告書・議事メモ・QA表）は
// metadata.種別 にそのまま残るため読み取りには影響しないが、新規登録では
// この5つしか選べない。議事録は本来ここではなく /api/meetings で
// 「会議」として登録すべきもの（lib/organizations.ts 参照）。
const DOC_TYPES = ["メモ", "実施理由書", "スライド", "提案書", "その他"];
